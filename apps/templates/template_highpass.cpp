...
ap_int<16> filter(const window w, ap_int<16> m) {
    
    ap_int<16> x1, x2, x3, x4, sum;
#pragma HLS pipeline II=1
    x1 = mul16s_"mul1"(w.pix[1][1], m);
    x2 = (ap_int<16>)-add16se_"add1"((ap_int<16>)-x1, w.pix[0][1]);
    x3 = (ap_int<16>)-add16se_"add2"((ap_int<16>)-x2, w.pix[1][0]);
    x4 = (ap_int<16>)-add16se_"add3"((ap_int<16>)-x3, w.pix[1][2]);
    sum = (ap_int<16>)-add16se_"add4"((ap_int<16>)-x4, w.pix[2][1]);
...
#pragma HLS DATAFLOW
	...
	Filter2D(m, width, height, window_stream, output_stream);
