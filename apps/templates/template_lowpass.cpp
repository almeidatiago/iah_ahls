...
ap_int<16> filter(const window w, ap_int<16> m1, ap_int<16> m2) {
    
    ap_int<16> x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, sum;
#pragma HLS pipeline II=1
    x1 = add16se_"add1"(w.pix[0][0], w.pix[0][2]);
    x2 = mul16s_"mul1"(w.pix[0][1], m2);
    x3 = add16se_"add1"(x1, x2);
    x4 = mul16s_"mul1"(w.pix[1][0], m2);
    x5 = mul16s_"mul2"(w.pix[1][1], m1);
    x6 = mul16s_"mul1"(w.pix[1][2], m2);
    x7 = add16se_"add2"(x4, x5);
    x8 = add16se_"add2"(x6, x7);
    x9 = add16se_"add3"(w.pix[2][0], w.pix[2][2]);
    x10 = mul16s_"mul1"(w.pix[2][1], m2);
    x11 = add16se_"add3"(x9, x10);
    x12 = add16se_"add3"(x3, x8);
    sum = add16se_"add3"(x11, x12);
...
#pragma HLS DATAFLOW
	...
	Filter2D(m1, m2, width, height, window_stream, output_stream);
