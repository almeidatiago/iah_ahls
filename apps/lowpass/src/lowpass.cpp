...
//#define HEIGHT 512
//#define WIDTH 512
#define COMMENT "Lowpass filter"
#define RGB_COMPONENT_COLOR 255

void lowpass(unsigned char* in, unsigned char* out, int HEIGHT, int WIDTH, int mul1, int mul2, int add1, int add2, int add3) {
    ...
    loop1: for (int i = 0; i < (HEIGHT/8)*8; i++) {
        loop2: for (int j = 0; j < (WIDTH/8)*8; j++) {
            ...
            x1 = add[add1](in[(i - 1) * HEIGHT + (j - 1)], in[(i - 1) * HEIGHT + (j + 1)]);
            x2 = mul[mul1](in[(i - 1) * HEIGHT + (j)], 2);
            x3 = add[add1](x1, x2);
            x4 = mul[mul1](in[(i) * HEIGHT + (j - 1)], 2);
            x5 = mul[mul2](in[(i) * HEIGHT + (j)], 4);
            x6 = mul[mul1](in[(i) * HEIGHT + (j + 1)], 2);
            x7 = add[add2](x4, x5);
            x8 = add[add2](x6, x7);
            x9 = add[add3](in[(i + 1) * HEIGHT + (j - 1)], in[(i + 1) * HEIGHT + (j + 1)]);
            x10 = mul[mul1](in[(i + 1) * HEIGHT + (j)], 2);
            x11 = add[add3](x9, x10);
            x12 = add[add3](x3, x8);
            sum = add[add3](x11, x12);
...
#pragma omp parallel for shared(inputList, filesList)
    for (int i = 0; i < size; i++) {
        ...
        lowpass(inputImage, outImage, image->y, image->x, atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6]), atoi(argv[7]));
