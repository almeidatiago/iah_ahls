...
def baseline(pparams_dict, dict_app):
    r2 = subprocess.run([f"./apps/{dict_app['name']}/app", 
                         ...
                         f"{pparams_dict['a0']}", f"{pparams_dict['a1']}", f"{pparams_dict['a2']}"], stdout=subprocess.PIPE)
...
def old_lowpass(params, **kwargs):
    ...
    for case in input_images:
        if (kind != 'precise' or not os.path.exists('local-image/lowpass_' + kind + str(i) + '.png')):
            filtered_image = lowpass_kernel(params, case)
            filtered_image = filtered_image.astype(np.uint8)
            edge_image = Image.fromarray(filtered_image)
            edge_image.save('local-image/lowpass_' + kind + str(i) + '.png')
        values.append('local-image/lowpass_' + kind + str(i) + '.png')
        i += 1
...
def image_run(i):
    ...
    filtered_image = lowpass_kernel(comps, input_images[i])
    filtered_image = filtered_image.astype(np.uint8)
    edge_image = Image.fromarray(filtered_image)
    edge_image.save('local-image/lowpass_' + kind + str(i+1) + '.png')
    
    return 'local-image/lowpass_' + kind + str(i+1) + '.png'
    

def lowpass_parallel(params, **kwargs):
    global comps
    comps = params
    global input_images
    input_images = kwargs.get('input')
    ...
    if (kind != 'precise'):# or not os.path.exists('local-image/sobel_' + kind + str(i) + '.png')):
        ...
        values = lowpass(params, **kwargs)

    #print(values)
    return values

def lowpass_kernel(params, input_image):
  ...
  for i in range(rows):
      for j in range(columns):
          if ((i < 1) or (i > rows - 2) or (j < 1) or (j > columns - 2)):
              ...
              x3 = params['a0'].calc(x1, x2)
              x4 = params['m0'].calc(input_image[i][j - 1], 2)
              x5 = params['m1'].calc(input_image[i][j], 4)
              x6 = params['m0'].calc(input_image[i][j + 1], 2)
              x7 = params['a1'].calc(x4, x5)
              x8 = params['a1'].calc(x6, x7)
              x9 = params['a2'].calc(input_image[i + 1][j - 1], input_image[i + 1][j + 1])
              x10 = params['m0'].calc(input_image[i + 1][j], 2)
              x11 = params['a2'].calc(x9, x10)
              x12 = params['a2'].calc(x3, x8)
              out = params['a2'].calc(x11, x12)
...
def lowpass(params, inputfile):
    if (params['kind'] != 'precise'):
        r2 = subprocess.run(["./apps/lowpass/app", 
                            f"apps/lowpass/{inputfile}",
                            f"{params['kind']}",
                            f"{params['m0']}",
                            f"{params['m1']}",
                            f"{params['a0']}",
        ...
        for i in range(init, end + 1):
            files.append("local-image/512x512/" + str(i) + ".precise.low.ppm")
