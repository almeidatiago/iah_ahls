...
def baseline(pparams_dict, dict_app):
    r2 = subprocess.run([f"./apps/canny/app", 
                     f"apps/{dict_app['name']}/{dict_app['training']}",
                     f"{pparams_dict['kind']}",
                     f"{pparams_dict['m0']}", f"{pparams_dict['m1']}", f"{pparams_dict['m2']}", f"{pparams_dict['m3']}", f"{pparams_dict['m4']}", 
                     f"{pparams_dict['m5']}", f"{pparams_dict['m6']}", f"{pparams_dict['m7']}", f"{pparams_dict['m8']}",
                     f"{pparams_dict['a0']}", f"{pparams_dict['a1']}", f"{pparams_dict['a2']}", f"{pparams_dict['a3']}",
                     f"{pparams_dict['a4']}", f"{pparams_dict['a5']}", f"{pparams_dict['a6']}", f"{pparams_dict['a7']}",
                     f"{pparams_dict['a8']}", f"{pparams_dict['a9']}", f"{pparams_dict['a10']}", f"{pparams_dict['a11']}",
                     f"{pparams_dict['a12']}", f"{pparams_dict['a13']}", f"{pparams_dict['a14']}", f"{pparams_dict['a15']}"], stdout=subprocess.PIPE)
...
def compute_error(original, approximate):
    ...
    if psnr_value: 
        psnr_value = np.median(psnr_value)
    else: 
        psnr_value = 0
    
    return psnr_value, np.median(ssim_value), np.median(mse_noise)
    

def old_canny(params, **kwargs):
    ...
    for case in input_images:
        if (kind != 'precise' or not os.path.exists('local-image/canny_' + kind + str(i) + '.png')):
            filtered_image = canny_kernel(params, case)
            filtered_image = filtered_image.astype(np.uint8)
            edge_image = Image.fromarray(filtered_image)
            edge_image.save('local-image/canny_' + kind + str(i) + '.png')
        values.append('local-image/canny_' + kind + str(i) + '.png')
        i += 1
...
def image_run(i):
    ...
    filtered_image = canny_kernel(comps, input_images[i])
    filtered_image = filtered_image.astype(np.uint8)
    edge_image = Image.fromarray(filtered_image)
    edge_image.save('local-image/canny_' + kind + str(i+1) + '.png')
    
    return 'local-image/canny_' + kind + str(i+1) + '.png'
    

def canny_parallel(params, **kwargs):
    global comps
    comps = params
    global input_images
    input_images = kwargs.get('input')
    ...
    if (kind != 'precise'):# or not os.path.exists('local-image/canny_' + kind + str(i) + '.png')):
        ...
        values = cannyGx(params, **kwargs)

    #print(values)
    return values

def canny_kernel(params, input_image):

  [rows, columns] = np.shape(input_image)  # we need to know the shape of the input grayscale image
  canny_filtered_image = np.zeros(shape=(rows, columns))  # initialization of the output image array (all elements are 0)
  ...
  for i in range(rows):
      for j in range(columns):
          if ((i < 1) or (i > rows - 2) or (j < 1) or (j > columns - 2)):
              ...
              if sum > 255:
                sum = 255 
              canny_filtered_image[i][j] = sum

  return canny_filtered_image

def canny(params, inputfile):
    if (params['kind'] != 'precise'):
        r2 = subprocess.run(["./apps/canny/app", 
                            f"apps/canny/{inputfile}",
                            f"{params['kind']}",
                            f"{params['m0']}",
                            f"{params['m1']}",
                            f"{params['m2']}",
        ...
        for i in range(init, end + 1):
            files.append("local-image/512x512/" + str(i) + ".precise.canny.ppm")
